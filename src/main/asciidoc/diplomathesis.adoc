...
As we wanted to reproduce the results ourselves, we created a simple benchmark to test scaling performance. We started out writing a lambda function which would return 50k bytes every time it is called. To prevent AWS from caching the result, we appended a random number to our return string and to speed up.
...
<<<
So our load testing program would look something like so:

```c
const string URL = "https://cuhtt9xal1.execute-api.eu-central-1.amazonaws.com/prod";
...
Furthermore, one of our biggest learning aspects in the project was the porting of already existing backend code. When we had to switch from microservices to serverless, we also had to port and rewrite already existing parts of the application. Having to do so gave us a greater understanding of the both of the used backend technologies.
